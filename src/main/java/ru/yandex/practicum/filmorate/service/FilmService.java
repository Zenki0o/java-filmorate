...
public class FilmService {
    ...
    public void removeLike(Integer filmId, Integer userId) {
        ...
        if (!film.getLikes().contains(userId)) {
            log.debug("Пользователь id={} не ставил лайк фильму id={}, удаление пропущено", userId, filmId);
            return;
        }
    ...
    public List<Film> getPopularFilms(int count) {
        if (count < 0) {
            throw new IllegalArgumentException("Передан отрицательный параметр count.");
        }
        ...
        log.debug("Запрошен список популярных фильмов (count={}), возвращено {} фильмов", count, popularFilms.size());
        return popularFilms;
    }
}
